...
impl UserEventExt for Octocrab {
    async fn list_user_events(&self, user: &String) -> Result<Vec<models::events::Event>> {
        let result: Result<Page<models::events::Event>> = self
...
fn process_event(event: models::events::Event) {
    match event.payload.unwrap().specific.unwrap() {
            ...
            match action {
                Some(action_string) => {
                    println!(
                        "\tPull request #{} by {} - {} - {}",
                        payload.number,
                        payload.pull_request.user.unwrap().login,
                        action_string,
                        payload.pull_request.title.unwrap()
                    )
                }
                None => {}
            }
        ...
        EventPayload::IssueCommentEvent(payload) => {
            let issue_type = if payload.issue.pull_request.is_some() {
                "pull request"
            } else {
                "issue"
            };
...
async fn main() -> Result<()> {
    ...
    events
        ...
        .sorted_by(|a, b| {
            let date_ordering = Ord::cmp(&a.created_at.with_timezone(&Local).date_naive(), &b.created_at.with_timezone(&Local).date_naive());
            match date_ordering {
